...
config.read("config_pure_100.ini")
...
class_weight = _train[['classID',"weight"]].groupby('weight').sum().to_dict()


_train, _validation = test_train_split(_train, split=0.1)
#TODO Define class weight
...
def voting_prediction(clfs, x_test):
    """
    """
    preds_ = None
    i = 0
...
#preds_df  = voting_prediction(gen_met_trainin.clfs, X_test)
